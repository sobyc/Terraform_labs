...
output "id" {
  value = data.azurerm_resource_group.rg-hub.id
}
...
output "virtual_network_name" {
  value = data.azurerm_virtual_network.vnet-hub.name
}
...
resource "azurerm_public_ip" "pip-vng-01" {
  name                = "pip-vng-hub-ci-01"
...
resource "azurerm_virtual_network_gateway" "vng" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.pip-vng-01.id
